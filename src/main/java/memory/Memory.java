...
public class Memory implements MemoryInterface {
    ...
    public String seg_load(int segIndex) {
        ...
        if (Memory.PAGE) {
            ...
            char[] data = disk.read(String.valueOf(disk_base), len);
    ...
    public String page_load(int segIndex, int pageNO) {
        ...
        int load_tlb = (endFrame - startFrame) / 2;
        if (endFrame == startFrame) load_tlb = 1;
        if (!TLB.isAvailable) load_tlb = 1;
        if (load_tlb > (TLB.TLB_SIZE_B / TLB.LINE_SIZE_B)/2) load_tlb = (TLB.TLB_SIZE_B / TLB.LINE_SIZE_B)/2;
        char[] data = disk.read(t.intToBinary(String.valueOf(pageNO * PAGE_SIZE_B)), PAGE_SIZE_B * load_tlb);
        boolean loaded = false;
        int reversePageNo = 0;
        String returnAddr = "";
        for (int i = pageNO + load_tlb - 1; i >= pageNO; i--) {

            for (int j = startFrame; j <= endFrame; j++) {   // 每个反向页表遍历，找到空闲的就加载进来
                if (!(reversedPageTbl(j).isValid)) {        // 该页空闲，没有被占用
                    reversePageNo = j;
                    loaded = true;
                    break;
                }
            ...
            if (i == pageNO) returnAddr = frameAddr;
            write(frameAddr, PAGE_SIZE_B, Arrays.copyOfRange(data, (i - pageNO) * PAGE_SIZE_B, (i - pageNO + 1) * PAGE_SIZE_B));
            ReversedPageItem reversedPageItem = reversedPageTbl(reversePageNo);
            reversedPageItem.vPageNO = i;
            reversedPageItem.isValid = true;
            reversedPageItem.updateTimeStamp();
            PageItem pageItem = pageTbl(i);
            pageItem.isInMem = true;
            pageItem.setFrameAddr(frameAddr.toCharArray());
            tlb.writeTLB(transformer.intToBinary(String.valueOf(i)).toCharArray(), frameAddr.toCharArray());
    ...
    private int removePageByLRU(int startFrame, int endFrame) {
        ...
        int tlbN = tlb.isMatch(reversedPageTbl[minIndex].vPageNO);
        if (tlbN != -1) tlb.invalid(tlbN);
